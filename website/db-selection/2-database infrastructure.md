...
![img](/img/db-selection/2-image5.png)
...
这种场景下，通过 Binlog 同步，把数据拉到 TiDB 里面，在这个集群上我们进行大量计算、高频点查的操作。对于比较大的数据量来说，写入量也是非常高的，TiDB 底层的存储 RockDB 采用 LSM-Tree 模型，对写入来说是一个友好的数据结构。所以这种场景 TiDB 写入可以满足我们的需求。
