---
title: 在 TiDB 中正确使用索引，性能提升 666 倍
hide_title: true
---
...
系统在TiDB上跑起来后，通过Dashboard观察到有一条SQL非常规律性地出现在慢查询页面中，打开SQL一看只是个单表查询并不复杂，感觉必有蹊跷。 ![企业微信截图_20220428135224.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220428135224-1651305546299.png)



## 问题现象

以下是从Dashboard中抓出来的原始SQL和执行计划，总共消耗了1.2s，其中绝大部分时间都花在了Coprocessor扫描数据中： ![企业微信截图_20220428151936.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220428151936-1651305575260.png)
...
![企业微信截图_20220428174049.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220428174049-1651305641343.png)
...
![企业微信截图_20220428181516.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220428181516-1651305694018.png)

**从这个执行计划看到现在已经变成了全表扫描，但是执行时间却比之前缩短了一倍多**，而且当命中Coprocessor Cache的时候那速度就更快了： ![企业微信截图_20220428181854.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220428181854-1651305714591.png)正当我觉得删掉索引就万事大吉的时候，监控里的Duration 99线突然升高到了200多ms，满脸问号赶紧查一下慢日志是什么情况。 发现这条SQL执行时间虽然变短了，但是慢SQL突然就变多了： ![企业微信截图_20220428152838.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220428152838-1651305737377.png)仔细对比SQL后发现，这些SQL是分别查询了`group_id`的6个值，而且频率还很高。也就是说除了前面贴出来的那条SQL变快，其他`group_id`的查询都变慢了。
...
![企业微信截图_20220429100406.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220429100406-1651305821926.png)
...
![企业微信截图_20220429144109.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220429144109-1651305917462.png)
...
![企业微信截图_20220430150018.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220430150018-1651306013012.png)

这个结果好像和预期的不太对呀，怎么搞成扫描31万行索引了？

别忘了，复合索引有个最左匹配原则，而这个`pre_excutetime`刚好是范围查询，所以实际只用到了`pre_excutetime`这个索引，而偏偏整个表的数据都符合筛选的时间段，其实就相当于`IndexFullScan`了。 那行，再把字段顺序换个位置：
...
![企业微信截图_20220430151241.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220430151241-1651306068072.png)看到执行时间这下满足了，在没有使用Coprocessor Cache的情况下执行时间也只需要1.8ms。**一个小小的索引调整，性能提升666倍**。 ![企业微信截图_20220430151523.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220430151523-1651306092640.png)
...
![企业微信截图_20220430153205.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220430153205-1651306166504.png)

![企业微信截图_20220430153252.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220430153252-1651306180585.png)

真实业务中，`flag=0`的数据不会超过50行，参考上面的结果，50次回表也就10ms以内，性能依然不错，完全符合要求。 我觉得应用层面允许调整SQL的话，再限制下`pre_excutetime`的最小时间，就可以算是个最好的解决方案了。

最后上一组图看看优化前后的对比。

![企业微信截图_20220428135721.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220428135721-1651306227355.png)

![企业微信截图_20220428135859.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220428135859-1651306240642.png)

![企业微信截图_20220428140051.png](https://tidb-blog.oss-cn-beijing.aliyuncs.com/media/企业微信截图_20220428140051-1651306274047.png)
