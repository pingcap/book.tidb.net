...
- TiDB/TiKV/TiFlash 参数支持在线修改
  ...
  - TiDB 将部分配置项转化为系统变量，所有变量支持在线变更，并支持持久化。请注意，转化后，原有配置项将被废弃。详细变更列表请查看[配置文件参数](https://docs.pingcap.com/zh/tidb/stable/release-6.1.0#配置文件参数)。
...
| TiDB     | [`enable-global-kill`](https://docs.pingcap.com/zh/tidb/stable/tidb-configuration-file#enable-global-kill-从-v610-版本开始引入) | 新增     | 当该配置项值默认为 `true` ，`KILL` 语句和 `KILL TIDB` 语句均能跨节点终止查询或连接，无需担心错误地终止其他查询或连接。 |
| TiDB     | [`enable-stats-cache-mem-quota`](https://docs.pingcap.com/zh/tidb/stable/tidb-configuration-file#enable-stats-cache-mem-quota-从-v610-版本开始引入) | 新增     | 控制 TiDB 是否开启统计信息缓存的内存上限。                   |
| TiKV     | [`raft-engine.enable`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#enable-1) | 修改     | 默认值从 `false` 修改为 `true`。                             |
| TiKV     | [`region-max-keys`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#region-max-keys) | 修改     | 默认值从 1440000 修改为 `region-split-keys / 2 * 3`。        |
| TiKV     | [`region-max-size`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#region-max-size) | 修改     | 默认值从 144 MB 修改为 `region-split-size / 2 * 3`。         |
| TiKV     | [`coprocessor.enable-region-bucket`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#enable-region-bucket-从-v610-版本开始引入) | 新增     | 是否将 Region 划分为更小的区间 bucket。                      |
| TiKV     | [`coprocessor.region-bucket-size`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#region-bucket-size-从-v610-版本开始引入) | 新增     | 设置 `enable-region-bucket` 启用时 bucket 的预期大小。       |
| TiKV     | [`causal-ts.renew-batch-min-size`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#renew-batch-min-size) | 新增     | 时间戳缓存的最小数量。                                       |
| TiKV     | [`causal-ts.renew-interval`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#renew-interval) | 新增     | 刷新本地缓存时间戳的周期。                                   |
| TiKV     | [`max-snapshot-file-raw-size`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#max-snapshot-file-raw-size-从-v610-版本开始引入) | 新增     | 当 snapshot 文件大于该配置项指定的大小时，snapshot 文件会被切割为多个文件。 |
| TiKV     | [`raft-engine.memory-limit`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#memory-limit) | 新增     | 指定 Raft Engine 使用内存的上限。                            |
| TiKV     | [`storage.background-error-recovery-window`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#background-error-recovery-window-从-v610-版本开始引入) | 新增     | RocksDB 检测到可恢复的后台错误后，所允许的最长恢复时间。     |
| TiKV     | [`storage.api-version`](https://docs.pingcap.com/zh/tidb/stable/tikv-configuration-file#api-version-从-v610-版本开始引入) | 新增     | TiKV 作为 Raw Key Value 存储数据时使用的存储格式与接口版本。 |
| PD       | [`schedule.max-store-preparing-time`](https://docs.pingcap.com/zh/tidb/stable/pd-configuration-file#max-store-preparing-time-从-v610-版本开始引入) | 新增     | 控制 store 上线阶段的最长等待时间。                          |
| TiCDC    | [`enable-tls`](https://docs.pingcap.com/zh/tidb/stable/manage-ticdc#sink-uri-配置-kafka) | 新增     | 控制是否使用 TLS 连接 Kafka。                                |
| TiCDC    | `sasl-gssapi-user` `sasl-gssapi-password` `sasl-gssapi-auth-type` `sasl-gssapi-service-name` `sasl-gssapi-realm` `sasl-gssapi-key-tab-path` `sasl-gssapi-kerberos-config-path` | 新增     | 支持 Kafka SASL/GSSAPI 认证所需要的参数。详情见 [Sink URI 配置 `kafka`](https://docs.pingcap.com/zh/tidb/stable/manage-ticdc#sink-uri-配置-kafka)。 |
| TiCDC    | [`avro-decimal-handling-mode`](https://docs.pingcap.com/zh/tidb/stable/manage-ticdc#sink-uri-配置-kafka) [`avro-bigint-unsigned-handling-mode`](https://docs.pingcap.com/zh/tidb/stable/manage-ticdc#sink-uri-配置-kafka) | 新增     | 控制 Avro 格式的输出细节。                                   |
| TiCDC    | [`dispatchers.topic`](https://docs.pingcap.com/zh/tidb/stable/manage-ticdc#同步任务配置文件描述) | 新增     | 控制 TiCDC 将增量数据分发到不同 Kafka Topic 的策略           |
| TiCDC    | [`dispatchers.partition`](https://docs.pingcap.com/zh/tidb/stable/manage-ticdc#同步任务配置文件描述) | 新增     | `dispatchers.partition` 是原 `dispatchers.dispatcher` 配置项的别名，用于控制增量数据的 Kafka Partition 分发策略。 |
| TiCDC    | [`schema-registry`](https://docs.pingcap.com/zh/tidb/stable/manage-ticdc#ticdc-集成-kafka-connect-confluent-platform) | 新增     | 用于指定存储 Avro Schema 的 Schema Registry Endpoint。       |
